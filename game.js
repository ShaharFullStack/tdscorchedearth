...
export class Game {
  ...
  setupEventListeners() {
    // Only setup keyboard controls on desktop
    if (!this.isMobile) {
  ...
  handleMouseMove(event) {
    ...
    this.player.rotateTurretToward(mouseX * 0.1);
    this.updateCameraToPlayerView();
  }
  ...
  nextTurn() {
    ...
    if (this.isPlayerTurn) {
      ...
      const controlMsg = this.isMobile ? 
        ...
        "Your turn! Use arrow keys to aim, Space to fire";
  ...
  restart() {
    ...
    // Reset UI
    this.ui.updateMessage("Your turn! Use controls to aim and fire");
    this.ui.updatePowerMeter(this.player.getPower());
    this.ui.updateWindIndicator(this.windStrength);
    this.ui.updateHealthBars(this.player.health, this.enemies.map(e => e.health));
